...
direct paths with paths mediated by the regional integration gateway. The RDG
framework provides the artefact that connects these objectives. Its evidence chain preserves
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|
            ...
            \hline
            Compare direct paths with paths mediated by the regional
            integration gateway. &
            Path-level Overhead
            $\Delta L(P_{50})=L_{\mathrm{mediated}}-L_{\mathrm{direct}}$. &
            Successful-flow medians of $755.8$~ms and $571.1$~ms yield
...
differential measurement because internal regional integration gateway telemetry lies outside the
...
to regional integration gateway mediation.
...
\noindent The RDG framework preserves a strict execution invariant: steps within one flow
...
index $\tau$ identify the first unsuccessful transition. The Traffic Execution Engine
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Empirically annotated workflow automaton]{Empirically
...
\noindent At the regional integration gateway boundary, suppression of an invalid suffix removes requests
...
property; it neither reveals regional integration gateway capacity nor eliminates transfer in vain
for requests that already enter a long queue or approach a timeout.

\subsection[Queueing Saturation and Load-Sharing Window]{Queueing Saturation and Load-Sharing Window}
\label{subsec:conclusions-queueing-saturation}

\noindent The evaluation protocol asks whether the experimental method exposes
...
Flow occurrences continue to arrive, but the Traffic Execution Engine cannot admit them at
their planned instants; delivery drift therefore accumulates outside the
semaphore.

\noindent The plateau identifies the RDG framework's configured concurrency boundary, not
...
logs from the Application Profile Management System (APMS), regional integration gateway components,
and downstream services are inaccessible within the authorised security and
policy perimeter. The RDG framework therefore measures the
...
processing, queue residence, transformation, routing, regional integration gateway computation, and
downstream execution. Consequently, \emph{$\Delta L(P_{50})=184.7$~ms remains
an aggregate surrogate displacement}, not a direct measurement of regional integration gateway
service time.
...
\noindent The extreme campaign also profiles the limits of a single RDG node.
...
amount. The extreme run consequently characterises the coupled RDG--path
...
\noindent The engineering roadmap separates RDG capacity from regional integration gateway
...
\noindent The first evolution replaces the single-node RDG framework with a
Kubernetes-orchestrated cluster whose workers execute disjoint
...
Log Collector. If worker $k$ delivers intensity $\lambda_k(t)$, the intended offered
load is
\[
...
\noindent The second evolution extends the open-loop Timeline Generator with
constrained feedback control. At control instant $k$, the controller observes
\[
...
region for that knee rather than an unsupported exact regional integration gateway threshold. A
...
only end-to-end latency can misclassify RDG saturation as remote
saturation.
...
including autocorrelation, burst duration, concurrency, and flow proportions, and not through access to identifying clinical content. A segment
...
The non-stationary Poisson model of the Timeline Generator admits arbitrarily short inter-arrival
...
Traffic Execution Engine's lowered control-flow graph. Let $\Sigma$ be the concrete
...
the present Traffic Execution Engine can accumulate an unbounded waiting
population, so proving
